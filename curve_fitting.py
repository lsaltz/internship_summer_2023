...
from skimage import io, img_as_bool, color
...
if __name__ == '__main__':
    my_mask = np.asarray(color.rgb2gray(io.imread("mask0485.png")))
    my_depth = np.asarray(cv2.imread("depth0485.png"))
    my_depth2 = np.asarray(io.imread("depth0485.png"))
    detector = BezierBasedDetection(my_mask, use_medial_axis=True)
    depths_average = []
    curve = detector.fit()
    ...
    curve_pts, ts = curve.eval_by_arclen(ds, normalized=True)
    #pts = curve(ts)
    mask1 = cv2.polylines(my_depth, [curve_pts.reshape((-1, 1, 2)).astype(int)], False, (0, 200, 200), 1)
    depths = []
    for i in range(len(curve_pts)-1):
        bottom_pt = (curve_pts[i][0].astype(int)+ radii[i].astype(int), curve_pts[i][1].astype(int))
        bp = (curve_pts[i][0].astype(int) - radii[i].astype(int), curve_pts[i][1].astype(int))
        tp = (curve_pts[i+1][0].astype(int) + radii[i+1].astype(int), curve_pts[i+1][1].astype(int))
        top_pt = (curve_pts[i+1][0].astype(int) - radii[i+1].astype(int), curve_pts[i+1][1].astype(int))
        
        
        arr = np.array([bottom_pt, bp, top_pt, tp])
        img = cv2.drawContours(my_depth.copy(), [arr.astype(int)],-1, color=255, thickness=-1)
        pts = np.where(img==255)
        depths.append(my_depth2[pts[0], pts[1]])
        depths_average.append(np.mean(depths[i]))
        cv2.circle(mask1, curve_pts[i].astype(int), radii[i].astype(int), (0, 0, 255), thickness=1)
